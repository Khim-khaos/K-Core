...
KubekSettingsUI = class {
    ...
    static saveUser = () => {
        ...
        let body = { login, email, permissions, password };
        if (selectedServersInList.length > 0) {
            body.servers = selectedServersInList.join(",");
        }
        if (currentEditorMode === "new") {
            KubekRequests.put("/accounts", (result) => {
                KubekSettingsUI.hideUserEditor();
                if (result === true) {
            ...
            KubekRequests.put("/accounts/" + login, (result) => {
                KubekSettingsUI.hideUserEditor();
                if (result === true) {
