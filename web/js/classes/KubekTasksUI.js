...
const TASK_ITEM_CLICKABLE = "<div class='alert clickable-installer' data-id='$0' onclick='KubekInstallerConsole.open(\"$0\", \"$5\")'><div class='$1'>$2</div><div class='content-2'><span class='caption'>$3</span><span class='description'>$4</span></div></div>";

let isConnectionLost = false;

class KubekTasksUI {
    // Добавить новую задачу в список
    static addTask(id, icon, title, description, append = true, iconType = "symbol", iconBgClasses = "icon-bg colored", serverName = "") {
        let iconPrepared = "";
        if (iconType === "symbol") {
        ...
        let taskHTML;
        if (serverName) {
            taskHTML = TASK_ITEM_CLICKABLE.replaceAll("$0", id).replaceAll("$1", iconBgClasses).replaceAll("$2", iconPrepared).replaceAll("$3", title).replaceAll("$4", description).replaceAll("$5", serverName);
        } else {
            taskHTML = TASK_ITEM_PLACEHOLDER.replaceAll("$0", id).replaceAll("$1", iconBgClasses).replaceAll("$2", iconPrepared).replaceAll("$3", title).replaceAll("$4", description);
        }
        if (append === true) {
        ...
        $.ajax({
            ...
            success: (tasks) => {
                ...
                for (const [id, task] of Object.entries(tasks)) {
                    ...
                    if (task.type === KubekPredefined.TASKS_TYPES.CREATING && typeof task.serverName !== "undefined") {
                        ...
                        switch (task.currentStep) {
                            ...
                            case KubekPredefined.SERVER_CREATION_STEPS.DOWNLOADING_CORE:
                                ...
                                break;
                            case KubekPredefined.SERVER_CREATION_STEPS.INSTALLING:
                                description = "{{serverCreationSteps.installing}}";
                                clickableServerName = task.serverName;
                                if (typeof installerConsoleModals === "undefined" || installerConsoleModals[id] === undefined || installerConsoleModals[id] === false) {
                                    setTimeout(() => KubekInstallerConsole.open(id, task.serverName), 500);
                                }
                                icon = "terminal";
                                break;
                    ...
                    this.addTask(id, icon, title, description, true, "symbol", iconBg, clickableServerName);
                }
            },
