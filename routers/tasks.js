...
// Endpoint лога установки по ID задачи
router.get("/:id/log", function (req, res) {
    let q = req.params;
    const tasks = APP_CONFIG.getTasks();
    if (COMMONS.isObjectsValid(q.id) && Object.keys(tasks).includes(q.id) && tasks[q.id]) {
        res.set("Content-Type", "application/json");
        return res.send({ log: tasks[q.id].log || [] });
    }
    res.sendStatus(400);
});
