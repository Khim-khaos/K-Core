...
// Предупреждение, если авторизация отключена
if (mainConfig.authorization === false) {
    LOGGER.warning("Авторизация ОТКЛЮЧЕНА! Панель доступна всем, кто имеет доступ к сети. Настоятельно рекомендуется включить авторизацию в config.json (authorization: true).");
}
