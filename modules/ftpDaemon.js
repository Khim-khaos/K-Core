    ...
    if (isEnabled) {
        ...
        ftpServer.on("login", ({ connection, username: inputUsername, password: inputPassword }, resolve, reject) => {
            if (inputUsername === username && inputPassword === password) {
                LOGGER.log(
                    MULTILANG.translateText(mainConfig.language, "{{console.ftpConnected}}") + pc.green(username)
                );
                resolve({
                    root: initPath,
                    cwd: "/",
        ...
        ftpServer
            .listen()
            .then(() => {
                LOGGER.log(MULTILANG.translateText(mainConfig.language, "{{console.ftpStarted}}") + pc.cyan(port));
            })
            .catch((err) => {
                LOGGER.error(MULTILANG.translateText(mainConfig.language, "{{console.ftpError}}") + err.toString());
            });
