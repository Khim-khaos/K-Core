...
exports.logWebRequest = (req, res, username = null) => {
    ...
    let additionalInfo2 = "";
    if (username !== null) {
        additionalInfo2 = "[" + pc.cyan(username) + "]"
    }
    LOGGER.log("[" + pc.yellow(ip) + "]", additionalInfo2, pc.green(req.method) + " - " + req.originalUrl);
};
...
exports.csrfMiddleware = (req, res, next) => {
    ...
    // Если авторизация отключена, CSRF не имеет смысла
    if (APP_CONFIG.getMainConfig().authorization === false) {
        return next();
    }
    ...
    // Если заголовка нет, но есть сессия - это подозрительно
...
exports.startWebServer = () => {
    ...
    server.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            LOGGER.log(pc.red(`Ошибка: Порт ${webPort} уже занят. Пожалуйста, измените webserverPort в config.json.`));
        } else {
            LOGGER.log(pc.red(`Ошибка при запуске веб-сервера: ${err.message}`));
        }
        process.exit(1);
    });
};
