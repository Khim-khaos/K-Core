...
exports.addNewTask = (data) => {
    ...
    LOGGER.log(MULTILANG.translateText(mainConfig.language, "{{console.taskAdded}}", pc.cyan(newTaskID), pc.cyan(data.type)));
    return newTaskID;
};
...
exports.removeTask = (taskID) => {
    ...
    if (typeof tasks[taskID] !== 'undefined') {
        tasks[taskID] = null;
        delete tasks[taskID];
        LOGGER.log(MULTILANG.translateText(mainConfig.language, "{{console.taskRemoved}}", pc.cyan(taskID)));
        return true;
    }
    return false;
};
