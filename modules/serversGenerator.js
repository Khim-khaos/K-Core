...
async function startJavaServerGeneration(serverName, core, coreVersion, startParameters, javaExecutablePath, serverPort, cb) {
    const mainConfig = APP_CONFIG.getMainConfig();
    let coreDownloadURL = "";
    // Парсим версию: "1.20.1#123" -> cleanVersion="1.20.1", buildVersion="123"
    // Для NeoForge: "1.21.1#21.1.232" -> cleanVersion="1.21.1", neoforgeVersion="21.1.232"
    let cleanVersion = coreVersion;
    let buildVersion = null;
    if (coreVersion.includes('#')) {
    ...
    // Для NeoForge используем NeoForge-версию (21.1.232) вместо MC-версии (1.21.1)
    let coreFileVersion = (core.toLowerCase() === 'neoforge' && buildVersion) ? buildVersion : cleanVersion;
    let coreFileName = core + "-" + coreFileVersion + ".jar";
    ...
    if (isInstaller) {
        coreFileName = core + "-" + coreFileVersion + "-installer.jar";
    }
    ...
    LOGGER.log(`[Creation] Starting creation of server: ${pc.cyan(serverName)} (Core: ${core}, Version: ${coreVersion})`);

    // Если сервер с таким названием уже существует - не продолжаем
    if (SERVERS_MANAGER.isServerExists(serverName)) {
        LOGGER.warning(`[Creation] Server ${pc.red(serverName)} already exists!`);
    ...
    if (javaExecutablePath !== false) {
        ...
        // Удаляем устаревший кэш от старой схемы именования (NeoForge использовал MC-версию вместо NeoForge-версии)
        if (core.toLowerCase() === 'neoforge' && buildVersion) {
            const oldCachePath = path.join(coreCacheDir, core + "-" + cleanVersion + "-installer.jar");
            if (fs.existsSync(oldCachePath) && oldCachePath !== coreCachePath) {
                try {
                    fs.unlinkSync(oldCachePath);
                    LOGGER.log(`[Creation] Removed stale cache: ${oldCachePath}`);
                } catch (e) {
                    LOGGER.warning(`[Creation] Could not remove stale cache: ${e.message}`);
                }
            }
        }
        ...
        if (core.match(/\:\/\//gim) === null && fs.existsSync("./servers/" + serverName + path.sep + core)) {
            ...
            LOGGER.log(MULTILANG.translateText(mainConfig.language, "{{console.serverCreatedSuccess}}", pc.cyan(serverName)));
            safeCb(true);
        } else {
            ...
            CORES_MANAGER.getCoreVersionURL(core, coreVersion, (url, mirrors = []) => {
                if (url === false) {
                    ...
                    LOGGER.error(`[Creation] Please manually put the core file in: ${pc.yellow(path.resolve(coreCachePath))}`);
                    if (TASK_MANAGER.isTaskExists(creationTaskID)) {
                        tasks[creationTaskID].currentStep = PREDEFINED.SERVER_CREATION_STEPS.FAILED;
                    }
                    safeCb(false);
                ...
                DOWNLOADS_MANAGER.addDownloadTask(coreDownloadURL, serverCorePath, (coreDlResult) => {
                    if (coreDlResult === true) {
                        ...
                        LOGGER.error(`[Creation] You can manually put the core file in: ${pc.yellow(path.resolve(coreCachePath))} and try again.`);
                        safeCb(false);
                    }
                }, coreMirrors);
            });
...
function handleCoreReady(creationTaskID, serverName, core, coreFileName, startParameters, javaExecutablePath, serverPort, serverDirectoryPath, isInstaller, safeCb) {
    ...
    if (isInstaller) {
        ...
        let fullJavaPath = javaExecutablePath === "java" ? "java" : path.resolve(javaExecutablePath);
        
        const { spawn } = require('child_process');
        let installProcess = spawn(fullJavaPath, ['-jar', coreFileName, installerArgs], {
            cwd: serverDirectoryPath
        });
        
        let installerTimedOut = false;
        const INSTALLER_TIMEOUT = 10 * 60 * 1000;
        const installerTimeout = setTimeout(() => {
            installerTimedOut = true;
            LOGGER.warning("[Installer] Installer timed out after 10 minutes, killing process...");
            installProcess.kill('SIGTERM');
        }, INSTALLER_TIMEOUT);
        
        const appendInstallerLog = (text) => {
            if (tasks[creationTaskID]) {
                if (!tasks[creationTaskID].log) tasks[creationTaskID].log = [];
                tasks[creationTaskID].log.push(text);
                if (tasks[creationTaskID].log.length > 500) tasks[creationTaskID].log.shift();
                tasks[creationTaskID].description = text.substring(0, 120);
            }
        };
        ...
        installProcess.stdout.on('data', (data) => {
            ...
            if (output) {
                ...
                for (const line of lines) {
                    ...
                    if (trimmed) {
                        LOGGER.log("[Installer] " + trimmed);
                        appendInstallerLog(trimmed);
                    }
                }
            }
        });
        ...
        installProcess.on('exit', (code) => {
            ...
            if (code === 0 || code === null) {
                ...
                if (targetCoreJar) {
                    ...
                    tasks[creationTaskID].currentStep = PREDEFINED.SERVER_CREATION_STEPS.CREATING_BAT;
                    module.exports.writeJavaStartFiles(serverName, targetCoreJar, startParameters, javaExecutablePath, serverPort, core);
                    tasks[creationTaskID].currentStep = PREDEFINED.SERVER_CREATION_STEPS.COMPLETION;
                    serversConfig[serverName] = {
                        status: PREDEFINED.SERVER_STATUSES.STOPPED,
                        restartOnError: true,
                        maxRestartAttempts: 3,
